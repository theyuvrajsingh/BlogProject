...
# def myblogs(request,author):
#     mypost=BlogPost.objects.all()
#     user=BlogPost.objects.get(author_id=author)
#     mypost=BlogPost.objects.filter(user=user)
#     return render(request,"my_blogs.html",{'mypost':mypost})

def myblogs(request, author):
    user = get_object_or_404(User, username=author)
    myposts = BlogPost.objects.filter(author=user)
    return render(request, "my_blogs.html", {'myposts': myposts})
 


def catego(request,slug):# Function Based view  
    cat=category.objects.get(slug=slug)
    bpost=BlogPost.objects.filter(cat=cat)
    return render(request,"category.html",{'bpost':bpost,'cat':cat})
